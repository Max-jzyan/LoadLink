...
describe('trailerApi endpoints', () => {
  ...
  describe('listDriverTrailers', () => {
    it('[URL] GET driver/:driverId/trailers', async () => {
      ...
      await store.dispatch(api.endpoints.listDriverTrailers.initiate('driver-1'))
    ...
    it('[Success] data lands in cache', async () => {
      ...
      const store = createTestStore()
      await store.dispatch(api.endpoints.listDriverTrailers.initiate('driver-1'))
      const state = api.endpoints.listDriverTrailers.select('driver-1')(store.getState())
  ...
  describe('createTrailer', () => {
    it('[URL/Body] POST driver/:driverId/trailers with payload', async () => {
      ...
      await store.dispatch(api.endpoints.createTrailer.initiate({ driverId: 'driver-1', body: payload }))
      expect(lastCall().url).toContain('/api/driver/driver-1/trailers')
    ...
    it('[Success] data in result', async () => {
      mockFetchResponse({ status: 201, body: fixtures.trailer() })
      const store = createTestStore()
      const result = await store.dispatch(api.endpoints.createTrailer.initiate({ driverId: 'driver-1', body: {} as never }))
  ...
  describe('updateTrailer', () => {
    it('[URL/Body] PATCH driver/:driverId/trailers/:trailerId with payload', async () => {
      ...
      await store.dispatch(api.endpoints.updateTrailer.initiate({ driverId: 'driver-1', trailerId: 'trailer-1', body: payload }))
    ...
    it('[Success] data in result', async () => {
      mockFetchResponse({ status: 200, body: fixtures.trailer() })
      const store = createTestStore()
      const result = await store.dispatch(api.endpoints.updateTrailer.initiate({ driverId: 'driver-1', trailerId: 'trailer-1', body: {} }))
      expect(result.data?._id).toBe('trailer-1')
    })
  ...
  describe('deleteTrailer', () => {
    it('[URL] DELETE driver/:driverId/trailers/:trailerId', async () => {
      mockFetchResponse({ status: 200, body: {} })
      const store = createTestStore()
      await store.dispatch(api.endpoints.deleteTrailer.initiate({ driverId: 'driver-1', trailerId: 'trailer-1' }))
      expect(lastCall().url).toContain('/api/driver/driver-1/trailers/trailer-1')
    ...
    it('[Success] resolves', async () => {
      mockFetchResponse({ status: 200, body: {} })
      const store = createTestStore()
      const result = await store.dispatch(api.endpoints.deleteTrailer.initiate({ driverId: 'driver-1', trailerId: 'trailer-1' }))
