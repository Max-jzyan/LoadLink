...
export function RevenueSummaryCards({ revenue, viewMode = 'completed' }: { revenue: RevenueSummary | null; viewMode?: DashboardViewMode }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <DynamicCard title={isPotential ? 'Expected Revenue' : 'Total Revenue'} action={<DollarSign className="text-primary" />}>
        <p className="text-4xl font-bold">{animatedRevenue}</p>
      </DynamicCard>
      <DynamicCard title={isPotential ? 'Estimated Expenses' : 'Total Expenses'} action={<Fuel className="text-orange-500" />}>
        <p className="text-4xl font-bold">{animatedExpenses}</p>
      </DynamicCard>
      <DynamicCard title={isPotential ? 'Expected Profit' : 'Net Profit'} action={<ProfitIcon className={profitColor} />}>
        <p className={cn('text-4xl font-bold', profitColor)}>{animatedProfit}</p>
      </DynamicCard>
      <DynamicCard title={isPotential ? 'Expected Margin' : 'Profit Margin'} action={<MarginIcon className={marginColor} />}>
