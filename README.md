...
| **In-App Messaging** — per-load message threads between company and driver                                                                        | Partially working (NEW in M4) | Log in as either role → Messages (`/messages`) for the thread list, or the message icon on a load (`MessageButton` → `LoadMessagesDrawer`) for a specific thread. Backend: `messageController` + `messageService`. Note issues 245, 246.                |
