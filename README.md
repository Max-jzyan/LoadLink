...
| **Profile & Fleet Management** — driver profile, truck CRUD (add/edit/delete, set primary), trailer CRUD (add/edit/delete, set primary), certifications, carrier credentials (MC/DOT), notification preferences, profile picture upload | Fully working (UI NEW in M3)        | Log in as Driver → Profile (`/driver/profile`). `TruckDrawer`, `TrailerDrawer`, `DriverInfoDrawer`, `NotificationPreferencesCard`. Backend API at `/api/driver/:id` and `/api/driver/:id/trucks` + `/api/trailers` |
...
| **Driver Public Profile + Reviews/Ratings** — public-facing driver profile with star ratings, review submission                                                                                                                         | Fully working (NEW in M3)           | Navigate to `/driver/profile/:driverId` (linked via `DriverNameLink` components throughout app). Submit a review via `ReviewForm`                                                                                  |
